...
- **Choosing between several available issues?** Score them with the triage rubric in [`.claude/skills/triage-task/SKILL.md`](.claude/skills/triage-task/SKILL.md) — an advisory read of priority × value × token cost that returns a Do-now / Good-ROI / Defer / Skip verdict, so spare tokens go to the highest-value work. It's guidance only: it never claims, relabels, or reorders anything.
