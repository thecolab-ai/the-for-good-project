...
- **The shelf-level data exists, but public reuse is constrained.** Grocer's public data can support current prices, store coverage, and product histories, but the conservative finding is to publish methods, aggregate analysis, and small cited examples rather than bulk data or retailer accusations. (confidence: Medium; reuse-rights reading: Medium-Low) — [data audit](../research/findings/other/grocer-nz-data-audit.md)
...
- **Whether public outputs can safely use examples based on Grocer data.** The rights finding is a conservative non-lawyer reading, and Grocer has not granted explicit permission. This limits every public page or brief.
- **What the shopper actually saw at the shelf or online.** Historical price rows usually show date, store, and price, not the displayed "was", "now", loyalty, multibuy, shelf-ticket wording, or checkout result.
...
Signal: the clearest blocker for any public product-level page or brief is still data permission plus captured shelf or online claims.
