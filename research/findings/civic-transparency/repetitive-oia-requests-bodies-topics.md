...
- **Central-government OIA volume is extraordinarily concentrated.** In calendar 2025, the 111 agencies that report to the Public Service Commission (PSC) completed **151,619 OIA requests**; **New Zealand Police alone completed 64,058 (≈42%)**, and Police + the Natural Hazards Commission (former EQC) + the Department of Corrections together accounted for **~66%** ([PSC OIA statistics, Jul–Dec 2025 & Jan–Jun 2025 datasets](https://www.publicservice.govt.nz/guidance/official-information/oia-statistics); my per-agency parse of the two workbooks is committed as [`psc-oia-2025-totals.csv`](../../data/oia-title-sample/psc-oia-2025-totals.csv), auditable row-by-row against the public workbooks). PSC stats **exclude local councils** (they operate under LGOIMA and report separately).
...
- **On the public FYI archive** (34,487 requests across 703 active bodies), the most-requested authorities are **NZ Police, Ministry of Health, MBIE, University of Otago, ACC**, plus heavy council representation (Auckland Council/Transport, Wellington, Christchurch) that is invisible in PSC stats ([FYI authority list](https://fyi.org.nz/body/list/all); the full per-authority count for **every** active body — from which the 34,487 total, the 703-body count, and the top-N shares below are all derived — is committed as [`authority-list-2026-07-03.json`](../../data/oia-title-sample/authority-list-2026-07-03.json) and regenerable via [`fyi-authority-list.mjs`](../../../scripts/research/fyi-authority-list.mjs)).
...
**Overall confidence:** Medium overall — the *which-bodies* volume picture is High (primary PSC data, parsed into a committed per-agency table); the *which-topics* picture is Medium (reproducible ~44% themed-share, committed sample); the *individualised-request tilt* and any precise *repetition rate* are **Low** (inferred / not pinnable from committable public metadata).
...
1. **PSC OIA statistics** — the official half-yearly collection of OIA requests *completed* by central-government agencies. I downloaded the per-period Excel workbooks `OIA-statistics-Jan-Jun-2025-FInal.xlsx` and `OIA-statistics-Jul-Dec-2025.xlsx`, plus the consolidated `v_OIAStatisticsAllDataResults` CSV (which runs 2015–2024, so the 2025 figures here come from the two workbooks), from the [PSC OIA statistics page](https://www.publicservice.govt.nz/guidance/official-information/oia-statistics) ([Wayback snapshot](https://web.archive.org/web/20260226192108/https://www.publicservice.govt.nz/guidance/official-information/oia-statistics)). **Reproduction:** in each workbook, per-agency completed counts are the "OIA requests completed" column (column C) of the *Timeliness & publication* sheet (Public Service departments and Crown entities) plus the *Police & NZDF – time & pub* sheet (Police and NZDF, which PSC breaks out separately); the category roll-up rows (agency-type totals, which have no agency name) are excluded. Summing the two half-year workbooks by agency gives the committed table [`psc-oia-2025-totals.csv`](../../data/oia-title-sample/psc-oia-2025-totals.csv) — 151,619 total across 111 agencies, which anyone can check against the public workbooks.
2. **fyi.org.nz** — the public OIA request archive (an [Alaveteli](https://fyi.org.nz/help/api) instance run by the NZ Council for Civil Liberties). I fetched FYI listing/index HTML pages that publicly display per-authority request counts, request titles, requester links, and short snippets. The committed artifacts **parse and store only counts, request IDs, titles, and authority slugs/URLs**; they do **not** store request bodies, correspondence, requester names, requester profile links, or snippets. Personal names of apparently-private individuals are redacted in the preserved titles; public-interest named matters are retained (same rule as the sibling council finding). The FYI analysis is **committed and reproducible** in two parts, so a reviewer can audit the numbers rather than trust a scrape:
   - **Corpus-wide counts** (34,487 requests, 703 active bodies, top-N shares, and the top-20 table) come from the full paginated authority index. Every active authority and its request count is committed as [`authority-list-2026-07-03.json`](../../data/oia-title-sample/authority-list-2026-07-03.json), regenerable via [`scripts/research/fyi-authority-list.mjs`](../../../scripts/research/fyi-authority-list.mjs).
   - **Title-level analysis** (topic clusters, recurrence, cross-body groups) comes from a per-body title sample regenerated by [`scripts/research/fyi-oia-title-sample.mjs`](../../../scripts/research/fyi-oia-title-sample.mjs); the raw sample — every sampled title, with FYI request IDs, plus the cluster/recurrence/cross-body tallies — is preserved in [`sample-2026-07-03.json`](../../data/oia-title-sample/sample-2026-07-03.json). See the [data note](../../data/oia-title-sample/README.md).
...
I did **not** access the de-identification pipeline scoped in the sibling method issue (#200, whose feasibility assessment landed in #206) — so this finding stays strictly on counts, titles, and normalised title-templates, which are safe to aggregate.
...
Totals: **151,619** across 111 reporting agencies (110 with at least one completed request); **top 3 = 66%**, **top 10 = 85%** ([PSC OIA statistics](https://www.publicservice.govt.nz/guidance/official-information/oia-statistics); [committed `psc-oia-2025-totals.csv`](../../data/oia-title-sample/psc-oia-2025-totals.csv)). Police and NDF are reported separately in the PSC files and excluded from its "Public Service" summary headline — which is why external summaries of the PSC release quote a much smaller number: [RNZ, reporting a PSC half-year release](https://www.rnz.co.nz/news/national/497922/more-than-20-000-official-information-requests-so-far-this-year-with-oranga-tamariki-responding-the-slowest), states "**99 agencies completed 26,505 OIA requests between January and June**" (a *Public-Service-excluding-Police* figure) and notes that "**Department of Corrections and Toka Tū Ake EQC received the most requests… with over 5000 each**." RNZ therefore corroborates the exclusion convention and that Corrections/EQC were high-volume in that earlier Public Service subset; it does **not** corroborate the 2025 Police-inclusive ranking or the exact top-three share.

This concentration is structurally visible in how the data is reported: PSC breaks **Police and NZDF out onto separate sheets** and excludes them from its headline "Public Service" summary, and Police also [publish their own OIA statistics](https://www.police.govt.nz/police-latest-oia-statistics) separately. (Police's 64,058 does numerically dwarf every other agency in the PSC data — the next-largest is 18,968 — but the sources support only that Police are reported separately, not a stated *reason* for it.) The Ombudsman has published a dedicated OIA-practice review of [Corrections (2022)](https://www.ombudsman.parliament.nz/resources/oia-compliance-and-practice-department-corrections-2022), and RNZ separately reported that [Police and Corrections topped OIA-complaint tables](https://www.rnz.co.nz/news/national/323503/police,-corrections-have-most-oia-complaints).
...
FYI corpus = **34,487 requests** across 703 active authorities (≈34.5k is also the ceiling implied by the highest request ID observed, ~34,400). Top authorities ([FYI authority list](https://fyi.org.nz/body/list/all), scraped 2026-07-03; **every** authority and count in the table below is committed in [`authority-list-2026-07-03.json`](../../data/oia-title-sample/authority-list-2026-07-03.json), so the totals and shares here are auditable line-by-line):
...
|---|---|---|---|
| NZ Police is **by far the single largest** OIA-receiving agency | [PSC OIA statistics datasets](https://www.publicservice.govt.nz/guidance/official-information/oia-statistics) (primary; my parse: 64,058 of 151,619, committed as [`psc-oia-2025-totals.csv`](../../data/oia-title-sample/psc-oia-2025-totals.csv)) | *The qualitative "largest by far" is corroborated:* [Police publish their OIA stats separately](https://www.police.govt.nz/police-latest-oia-statistics) and PSC breaks Police out onto a separate sheet excluded from its Public Service summary, and RNZ notes [Police & Corrections top OIA complaints](https://www.rnz.co.nz/news/national/323503/police,-corrections-have-most-oia-complaints). **The precise ≈42% share is single-sourced (PSC workbooks only)** — neither second source states it. | High for "largest"; ≈42% flagged single-source (PSC) |
| Three bodies (Police, EQC/Natural Hazards Commission, Corrections) field **~two-thirds** of central-govt OIA volume | [PSC data](https://www.publicservice.govt.nz/guidance/official-information/oia-statistics) (my parse: 100,050/151,619 = 66%, committed as [`psc-oia-2025-totals.csv`](../../data/oia-title-sample/psc-oia-2025-totals.csv)) | *Direction only:* RNZ on a PSC release says "[Corrections and Toka Tū Ake EQC received the most requests… over 5000 each](https://www.rnz.co.nz/news/national/497922/more-than-20-000-official-information-requests-so-far-this-year-with-oranga-tamariki-responding-the-slowest)" — corroborating the *concentration/ranking* but **not the 66% number**, and that release *excludes* Police. **The exact ~66% is single-sourced (PSC primary).** | High for concentration; exact ~66% flagged single-source (PSC) |
...
| A verifiable **cross-body campaign** exists: identical title "Use of small desk fans" sent to **≥6 agencies** | [FYI search results for "desk fans"](https://fyi.org.nz/search/desk%20fans/newest) (MBIE, Justice, Education, MSD, Oranga Tamariki, Health on page 1) | The same six requests are independently visible on each agency's own FYI request page (the search hits and the per-agency listings are two views of the same public records). *Not in the per-body title sample — those six agencies are surfaced by full-text search, not by the ten-body sample.* | High (directly observable, two views) |
...
10. Committed data artifacts and reproducible scripts (this PR), all accessed/generated 2026-07-03:
    - PSC 2025 per-agency completed totals — `research/data/oia-title-sample/psc-oia-2025-totals.csv` (parsed from the two PSC workbooks; sums to 151,619).
    - Full FYI authority list with per-body counts — `research/data/oia-title-sample/authority-list-2026-07-03.json` and `scripts/research/fyi-authority-list.mjs` (703 active bodies, 34,487 requests).
    - FYI title sample — `research/data/oia-title-sample/sample-2026-07-03.json` and `scripts/research/fyi-oia-title-sample.mjs` (795 sampled titles).
