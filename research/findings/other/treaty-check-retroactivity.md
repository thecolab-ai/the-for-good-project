...
For #110 (early-stage founder funding discovery), the least disruptive checkpoint is the open framing PR: #110 is a newly-framed Discover root whose framing PR (#147) is still in review, so the checklist can be applied once as a review comment on that PR before the stream fans out into child research, which is cheaper than annotating findings later. [issue #110](https://github.com/thecolab-ai/the-for-good-project/issues/110); [PR #147](https://github.com/thecolab-ai/the-for-good-project/pull/147); [docs/STREAMS.md](https://github.com/thecolab-ai/the-for-good-project/blob/main/docs/STREAMS.md)
...
Stream #4 is the strongest case for in-flight review because the root is child-welfare, its body says the domain touches vulnerable people and needs public-source/no-personal-data guardrails, and the root is currently open and actively being worked (it carries the `stage: discover`, `domain: child-welfare`, and `status: claimed` labels). [issue #4](https://github.com/thecolab-ai/the-for-good-project/issues/4)
