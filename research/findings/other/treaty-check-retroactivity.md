...
The current open stream roots, checked with `gh issue list --state open --label "stage: discover"` on 3 July 2026, include streams #2, #3, #4, #60, #61, #104, and #110. Each is at a different point in the pipeline, and the least disruptive checkpoint for each is simply its next already-scheduled human boundary: #2 and #60 have stream overview files in `streams/` that still carry `TODO(steward)` direction placeholders; #3 and #61 have merged overview/synthesis material and both now carry `status: awaiting-direction`, so their pending G1 direction decision is the boundary; #110 is a freshly-framed root whose framing PR (#147) is still in review; and #4 is an in-review root with a claimed child research issue still open and no overview file yet. Every one of these already has a natural human checkpoint ahead of it, so the retroactivity rule is uniform — attach the one-time Treaty-consideration check to that existing boundary rather than opening a new pass — and the per-stream notes below only differ in *which* boundary that is. [GitHub open Discover issues query](https://github.com/thecolab-ai/the-for-good-project/issues?q=is%3Aissue%20is%3Aopen%20label%3A%22stage%3A%20discover%22); [streams directory](https://github.com/thecolab-ai/the-for-good-project/tree/main/streams); [issue #3](https://github.com/thecolab-ai/the-for-good-project/issues/3); [issue #61](https://github.com/thecolab-ai/the-for-good-project/issues/61); [PR #147](https://github.com/thecolab-ai/the-for-good-project/pull/147); [issue #4](https://github.com/thecolab-ai/the-for-good-project/issues/4); [issue #25](https://github.com/thecolab-ai/the-for-good-project/issues/25)
...
For #61, the synthesis PR (#140) has now merged and the stream carries `status: awaiting-direction`, so the least disruptive checkpoint is the same as for #3: the steward folds the one-time Treaty check into the pending G1 direction decision, using the already-merged overview/synthesis as the surface, rather than reopening merged work. [PR #140](https://github.com/thecolab-ai/the-for-good-project/pull/140); [issue #61](https://github.com/thecolab-ai/the-for-good-project/issues/61); [docs/STREAMS.md](https://github.com/thecolab-ai/the-for-good-project/blob/main/docs/STREAMS.md)

For #3 (council spending legibility), the least disruptive checkpoint is the steward's pending G1 direction decision: #3 already carries `status: awaiting-direction`, so the check rides that existing human boundary — the steward simply records, when making the direction call, whether Treaty/tikanga/Māori-data questions bear on a civic-transparency stream — rather than adding a separate retroactive pass. [issue #3](https://github.com/thecolab-ai/the-for-good-project/issues/3); [docs/STREAMS.md](https://github.com/thecolab-ai/the-for-good-project/blob/main/docs/STREAMS.md)

For #110 (early-stage founder funding discovery), the least disruptive checkpoint is the open framing PR: #110 is a newly-framed Discover root whose framing PR (#147) is still in review, so the checklist can be applied once as a review comment on that PR before the stream fans out into follow-up research, which is cheaper than annotating findings later. [issue #110](https://github.com/thecolab-ai/the-for-good-project/issues/110); [PR #147](https://github.com/thecolab-ai/the-for-good-project/pull/147); [docs/STREAMS.md](https://github.com/thecolab-ai/the-for-good-project/blob/main/docs/STREAMS.md)
...
Stream #4 is the strongest case for in-flight review because the root is child-welfare, its body says the domain touches vulnerable people and needs public-source/no-personal-data guardrails, and the root is currently open and in review (it carries the `stage: discover`, `domain: child-welfare`, and `status: in-review` labels). [issue #4](https://github.com/thecolab-ai/the-for-good-project/issues/4)
...
26. The For Good Project, issue #3, "[Discover] Council spending is public but not legible to citizens", accessed 3 July 2026. https://github.com/thecolab-ai/the-for-good-project/issues/3
27. The For Good Project, issue #110, "[Discover] Early-stage NZ founders can't reach angel & seed funding", accessed 3 July 2026. https://github.com/thecolab-ai/the-for-good-project/issues/110
28. The For Good Project, PR #147, "research: frame NZ angel & seed funding discovery stream (Part of #110)", accessed 3 July 2026. https://github.com/thecolab-ai/the-for-good-project/pull/147
29. The For Good Project, issue #61, "[Discover] Supermarket price & promotion transparency for NZ households", accessed 3 July 2026. https://github.com/thecolab-ai/the-for-good-project/issues/61
