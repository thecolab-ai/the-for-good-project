...
- I used the vendored `nz-parliament` skill to define a 2025 sampling frame: 63 bills returned by `bills --keyword Amendment --all`, filtered to `status == "Royal Assent"` and `last_activity` in 2025. The skill reads the public bills.parliament.nz API, and the sampled cluster list is reproduced in [statutory_amendment_sample.py](data/statutory_amendment_sample.py). Source: [New Zealand Parliament bills API item example](https://bills.parliament.nz/v/6/7ae98561-af36-4dc6-fee0-08dd98c831fd).
- With seed `fg-676-2025-amendment-act-cluster-v1`, the five selected clusters were: Climate Change Response (Emissions Trading Scheme-Forestry Conversion), Social Security, Crimes Legislation (Stalking and Harassment), Marine and Coastal Area (Takutai Moana) (Customary Marine Title), and Judicature (Timeliness) Legislation Amendment Bills. The corresponding enacted legislation pages exist on legislation.govt.nz for the first four by title/Act number search and the fifth is fully retrievable as an official PDF. Sources: [Climate Change Response Amendment Act 2025](https://www.legislation.govt.nz/act/public/2025/52/en/latest/), [Social Security Amendment Act 2025](https://www.legislation.govt.nz/act/public/2025/25/en/latest/), [Crimes Legislation Amendment Act 2025](https://www.legislation.govt.nz/act/public/2025/72/en/latest/), [Marine and Coastal Area Amendment Act 2025](https://www.legislation.govt.nz/act/public/2025/58/en/latest/), [Judicature (Timeliness) Legislation Amendment Act 2025 PDF](https://www.legislation.govt.nz/act/public/2025/85/en/2025-12-19.pdf).
- Because legislation.govt.nz returned AWS WAF challenges to local XML/HTML/PDF fetches for most selected Acts, I completed the provision-level classification only for the one selected cluster whose official full text was readable end to end: the Judicature (Timeliness) Legislation Amendment Act 2025. The local `legislation-nz` `updates` command also failed with `upstream_parse_error`, and `get-act` returned `upstream_unexpected_html` against current XML URLs; this is a tooling/access limit, not evidence that the official pages are dead. Sources: [legislation-nz skill notes](../../../.skills/skills/legislation-nz/SKILL.md), [Judicature Act official PDF](https://www.legislation.govt.nz/act/public/2025/85/en/2025-12-19.pdf).
...
**Overall confidence:** Low — the within-Act classification is High confidence, but the population conclusion is deliberately Low because only one randomly selected cluster could be fully classified from official text in this run.
...
The frame was all rows returned by the vendored Parliament skill where the query `Amendment` found a bill, the API status was `Royal Assent`, and the `last_activity` timestamp began with `2025`. The command was:
...
That produced 63 2025 Royal Assent rows in this run, and the rows are embedded in the reproduction script so reviewers do not need the live API to reproduce the random draw. The skill documentation states that it tracks bills from the public bills.parliament.nz JSON API. Sources: [nz-parliament skill documentation](../../../.skills/skills/nz-parliament/SKILL.md), [New Zealand Parliament bills API item example](https://bills.parliament.nz/v/6/7ae98561-af36-4dc6-fee0-08dd98c831fd), [reproduction script](data/statutory_amendment_sample.py).

The cluster draw used Python's `random.Random(SEED).sample(FRAME, 5)` with seed `fg-676-2025-amendment-act-cluster-v1`. It selected:
...
The first four enacted Act pages were resolvable through web search, but local command-line retrieval of legislation.govt.nz XML/PDF pages returned an AWS WAF challenge rather than usable XML/text. The fifth selected Act's PDF was fully readable through the browser-backed web extraction path, so I treated it as the accessible selected cluster for the provision-level proof. Source: [Judicature Act 2025 PDF](https://www.legislation.govt.nz/act/public/2025/85/en/2025-12-19.pdf).
...
| The run cannot support a population-level "most NZ amendments are/are not repeals" estimate. | [Sampling script showing only one fully classified accessible cluster](data/statutory_amendment_sample.py) | [legislation-nz skill notes on upstream blocking and XML caveats](../../../.skills/skills/legislation-nz/SKILL.md) | High |
...
- A successful rerun with direct access to legislation.govt.nz XML/PDF for all five selected Acts would replace this one-cluster pilot with the intended five-cluster estimate.
...
- I could not verify whether the 63-row Parliament API frame exactly equals every 2025 enacted public Amendment Act, because the query is title-keyword based and may miss amendment Acts whose bill title lacks "Amendment" or include private/local Acts depending on the API result set.
