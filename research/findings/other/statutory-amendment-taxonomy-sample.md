...
- I used the vendored `nz-parliament` skill to define a 2025 sampling frame: 62 bills returned by `bills --keyword Amendment --all`, filtered to `status == "Royal Assent"` and `last_activity` in 2025. The skill reads the public bills.parliament.nz API, and the sampled cluster list is reproduced in [statutory_amendment_sample.py](data/statutory_amendment_sample.py). Sources: [New Zealand Parliament bills API item example](https://bills.parliament.nz/v/6/7ae98561-af36-4dc6-fee0-08dd98c831fd), [rework fetch log](data/statutory_amendment_fetch_log.txt).
