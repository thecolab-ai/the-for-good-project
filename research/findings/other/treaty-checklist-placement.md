...
- **This mirrors how the NZ Crown places its own Treaty check:** as a *standing analytical step embedded in the process* — across both the development process *and* the final product — with the *how-to questions living in a separate referenced guidance doc*, not duplicated into every template. Two independent official sources (DPMC and LDAC) support that split; the stronger *"start at the very beginning / most important precisely where interests aren't obvious"* framing is DPMC's specifically, and is marked as single-sourced in the claim table below.

**Overall confidence:** Medium — the repo-structure claims (what each doc contains today) are High and directly verifiable; the *core* NZ-precedent claim (Treaty consideration as a standing step across the development process **and** the final product, with the how-to kept in one referenced guidance doc) is High and double-sourced across DPMC and LDAC; the *stronger* DPMC-specific framing ("very beginning" / "more important where interests aren't obvious" / not a trigger) rests on DPMC alone and is marked as single-sourced below. The *recommendation itself* is a design judgement offered for a human steward, not a fact, so the headline is deliberately Medium.
...
New Zealand's official machinery has already solved the analogous placement problem, and it did so with a *split*: the **requirement** to do the analysis is embedded in the workflow, while the **how-to** lives in one referenced guidance document.
...
Two design lessons transfer directly: (1) place the check to bite at **both** the start (framing) **and** the output/decision point, not one or the other; (2) keep the *requirement* in the process and the *questions* in a single referenced guidance doc, rather than duplicating the questions everywhere.
...
- *For:* follows the repo's own established pattern exactly, so it is the least-confusing to a contributor who already knows the method; puts the check at both framing (method prose) and output (PR tick); one canonical statement, one referenced how-to — no lens duplication. Mirrors the Crown split above.
...
| NZ practice keeps the *requirement* in the workflow while the *how-to questions* live in one separate referenced guidance doc (not duplicated everywhere) | [DPMC — questions "set out in the Treaty of Waitangi guidance" / CO(19)5](https://www.dpmc.govt.nz/publications/co-19-5-te-tiriti-o-waitangi-treaty-waitangi-guidance) | [LDAC ch.5 supplies the analytical questions in one guideline chapter](https://www.ldac.org.nz/guidelines/legislation-guidelines-2021-edition/constitutional-issues-and-recognising-rights-2/chapter-5) | High |
