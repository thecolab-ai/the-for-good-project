...
| 5   | **Te Tiriti & Māori data sovereignty absent** | The first-listed domain is child welfare ([`docs/DOMAINS.md`](../docs/DOMAINS.md)), yet searches on 2 Jul 2026 found no mention of Te Tiriti, iwi partnership or Māori data sovereignty in tracked founding docs or public GitHub issues/PRs (see §9). External context makes this material for any child-welfare or Māori-data work: current Oranga Tamariki law requires strategic partnerships with iwi and Māori organisations for improving outcomes for Māori children and whānau ([Oranga Tamariki Act 1989, s 7](https://www.legislation.govt.nz/act/public/1989/24/en/latest/#DLM147088)), Te Mana Raraunga states Māori data should be subject to Māori governance ([Te Mana Raraunga](https://www.temanararaunga.maori.nz/)), and the government's Ngā Tikanga Paihere guidance is for safe, responsible and culturally appropriate data practice ([data.govt.nz](https://data.govt.nz/toolkit/data-ethics/nga-tikanga-paihere/)). | Open a Te Tiriti/Māori data-governance workstream. Treat "we need Māori partnership or authority we don't yet have" as a valid always-human finding ([`CONSTITUTION.md`](../CONSTITUTION.md) Article IV.2), not as something an agent can solve alone. |
...
Gaps 5 and 6 need their own sessions — the first-partner choice sidesteps them temporarily but doesn't resolve them. Gap 6 should land before a tool ships that an organisation depends on. Gap 5 should be treated as a precondition before any child-welfare stream, or any stream using Māori data, proceeds past framing unless a human with relevant domain authority records a different approach ([`CONSTITUTION.md`](../CONSTITUTION.md) Article IV.2). The replication ambition raises the bar on both: closed well once, they become kernel patterns other countries can adapt (§6).
...
| **Merits**          | Works from a cold start. Free to tackle important-but-unasked problems, including public-interest domains where this repo has not yet named a specific asker. Produces wide-reach public goods. Contributors can start immediately.                                                                   | Adoption has a clearer test — the asker is waiting, so "done = adoption" ([`MANIFESTO.md`](../MANIFESTO.md)) can be checked against their feedback. Relevance is tested earlier. A named asker may help rate-limit agent output to available human judgement, but that is an assumption to validate. |
| **Negatives**       | Nobody specific is waiting — adoption risks being left to later outreach. "Greatest" is judged from the inside. Unfalsifiable until a user is found. Burns scarce human judgement on work with no committed beneficiary. The open-PR backlog (28, snapshot 2 Jul 2026) is consistent with this risk, not proof of causation. | Cold-start dependent — no partner, no work; partner acquisition likely depends on human relationship-building. Scope bounded by whoever shows up; systemic problems with no natural asker can be missed. Risks becoming free consultancy for one org.                       |
...
_Proposed decisions, for the working group to ratify — not a record of decisions already taken. As of 2 July 2026 no broker is named and no partner charter exists in the repo (search 2 Jul 2026). The proposals below are: first partner = **one small NFP**; intake = a **named human broker** plus a **simple public form**, formalised by a lightweight **partner charter**. Any "loose contacts" are treated as unverified private context until a human records a non-sensitive public note._
...
| **Finding the partner** | Run as the project's own method: a Discover issue — "which NZ small NFPs have a research-shaped problem we could answer in 4–6 weeks?" Agents map the candidate landscape; humans identify and contact candidates. Grant-access domain first ([`docs/DOMAINS.md`](../docs/DOMAINS.md)); deliberately **not** child welfare until the Te Tiriti/Māori data-governance gap (§2, gap 5) has a human-approved approach.               |
...
**The legitimacy layer as a design slot, not an NZ footnote (proposed).** This is a normative design requirement, not an empirical claim about every country: each instance should document the local legitimacy question before sensitive-domain work opens. In NZ, Te Tiriti, iwi partnership and Māori data governance are the obvious unresolved slot for child-welfare and Māori-data work, supported by the external context in §2 gap 5. The proposal is consistent with the existing rule that sensitive domains need a human with domain authority ([`CONSTITUTION.md`](../CONSTITUTION.md) Article IV.2). NZ's answer would become an example others can adapt, not a template to copy blindly.
...
_Analysis and proposal. Some of this describes how the repo already works (cited). The WhatsApp/community-bot feedback channel is already recorded as the stream feedback decision ([`docs/STREAMS.md`](../docs/STREAMS.md); [`docs/adr/0001-streams-and-human-gates.md`](../docs/adr/0001-streams-and-human-gates.md)); the form, private-repo rule, and founding-partner sequencing below are proposed additions._
...
**Sequencing — don't build the bridge before there's traffic (proposed).** The feedback channel has already been chosen for streams: WhatsApp → community bot first, web form later ([`docs/STREAMS.md`](../docs/STREAMS.md); [`docs/adr/0001-streams-and-human-gates.md`](../docs/adr/0001-streams-and-human-gates.md)). This plan's sequencing claim is narrower: the founding-partner loop can start with a named broker while bot/form implementation catches up; whether that is enough for the whole founding-partner phase is an assumption to validate. Build the form alongside the demand side's public launch, and implement/expand the bot when feedback volume or stakeholder use justifies it. **Proposed rule:** sensitive material (partner contacts, unsigned drafts) lives outside the public repo until cleared for publication. A small private repo is one possible implementation, to be decided separately; the principle is to reconcile "open by default" ([`CONSTITUTION.md`](../CONSTITUTION.md) Article III.6) with not publishing private partner material.
...
| 3   | **Name the broker; open the founding-partner issue** | One person owns finding + signing the first small NFP. Agents map candidates; humans contact candidates and record non-sensitive context. Grant-access first; not child welfare.                                          | **This week**         | People       |
...
| 8   | **Open the Te Tiriti/Māori data-governance workstream** | State the gap plainly; begin identifying Māori advisors or partner orgs to co-author the legitimacy layer. Treat this as a precondition before child-welfare or Māori-data work proceeds unless a domain-authority human records a different approach ([`CONSTITUTION.md`](../CONSTITUTION.md) Article IV.2). | Start now — long pole | People       |

Items 1–4 are scoped to be started this week by a small group, not guaranteed to finish. Items 5–7 are housekeeping that would make the operating model's constraints visible. Item 8 is the long pole — start it now so it can inform the first sensitive-domain decision.
...
**Method.** Sources are the repo's own documents, live GitHub state, and selected external public sources for the Te Tiriti/Māori data-governance point. Claims of the form "the repo does X" are cited to the file. Claims of the form "the repo does _not_ do X" (absences) were checked by repo-wide text search on **2 July 2026** and are only as good as that snapshot.
...
- **No Te Tiriti / Māori data-sovereignty / iwi mention in founding docs or public GitHub issues/PRs** — search of `README`, `MANIFESTO`, `CONSTITUTION`, `docs/` returned zero hits, and `gh search issues` returned no results for `Te Tiriti`, `Māori`, `Maori`, `iwi`, `data sovereignty`, `sovereignty`, `Treaty`, or `partnership` in `thecolab-ai/the-for-good-project`. **Confidence: High** for the public repo as of the snapshot; **Medium** for the wider project, because private/off-repo conversations may exist.
...
**External context used for gap 5.** Current Oranga Tamariki law requires the chief executive to seek strategic partnerships with iwi and Māori organisations to improve outcomes for Māori children, young people and whānau ([Oranga Tamariki Act 1989, s 7](https://www.legislation.govt.nz/act/public/1989/24/en/latest/#DLM147088)). Te Mana Raraunga defines Māori data sovereignty as Māori data being subject to Māori governance and says its purpose includes asserting Māori rights and interests in relation to data ([Te Mana Raraunga](https://www.temanararaunga.maori.nz/); [Charter](https://www.temanararaunga.maori.nz/tutohinga)). Ngā Tikanga Paihere is government guidance for safe, responsible and culturally appropriate data use ([data.govt.nz](https://data.govt.nz/toolkit/data-ethics/nga-tikanga-paihere/)). **Confidence: Medium** for the operational recommendation; these sources show the issue is material, but they do not by themselves decide this project's governance answer.

**What is explicitly _not yet_ true.** The V4 mission is **not adopted** ([`README.md`](../README.md) still carries V1). The §5 "decisions" are **proposals** — no broker, charter, form or partner exists in the repo as of the snapshot. The §5 loose-contact assumption is unverified public-repo evidence. The §6 "proven" definition and §7 private-repo rule are **proposed**, not policy.
